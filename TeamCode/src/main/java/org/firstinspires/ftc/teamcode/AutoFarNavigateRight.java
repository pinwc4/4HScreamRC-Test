...
@Autonomous(name = "AutoFarNavigateRIGHT")

public class AutoFarNavigateRight extends OpMode {
    ...
    public void loop() {
        if (dcmFrontLeftMotor.getCurrentPosition() > -1534 && !bolGoRight) {
        ...
        if (dcmFrontLeftMotor.getCurrentPosition() > -975 && bolGoRight) {
            dcmFrontLeftMotor.setPower(1);
            dcmBackLeftMotor.setPower(-1);
            dcmFrontRightMotor.setPower(1);
            dcmBackRightMotor.setPower(-1);
